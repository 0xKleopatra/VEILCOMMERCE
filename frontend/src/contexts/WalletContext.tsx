...
import { createConnectedSession, type ConnectedSession, type DetectedWallet, listWallets } from '../lib/midnight';
    ...
    try {
      ...
      try {
        ...
        if (String(e?.message ?? '').toLowerCase().includes('network') || String(e).includes('networkId')) {
          api = await wallet.api.connect({ networkId: network });
      ...
      const zkBase = '/contract/veilcommerce';
      const sess = await createConnectedSession(api, zkBase);
